...
-- Draw the invisible FOV circle centered on the mouse position
local function drawInvisibleFov()
    ...
    if not fovCircle then
        ...
        fovCircle.Color = Color3.fromRGB(255, 0, 0)  -- Red color (you can leave this for debugging purposes, but it will be invisible)
    end

    -- Get the mouse position
    local mouseX, mouseY = getMousePosition()
    
    -- Position the circle at the mouse location and set the radius dynamically
...
RunService.RenderStepped:Connect(function()
    if isToggled then
        TriggerBotAction()
        drawInvisibleFov()  -- Draw the invisible FOV circle centered on the mouse
    end
end)
